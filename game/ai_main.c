...
qboolean isLame (bot_state_t *bs) 
{
	...
	if (bs->revengeEnemy 
		...
		&& bs->revengeEnemy->s.number == en->s.number
		&& bs->revengeHateLevel >= bs->loved_death_thresh) {
			return 0;
	}

	if (bs->frame_Enemy_Len < 82 && BG_InRoll(&en->client->ps, en->client->ps.legsAnim)) {
		return 0;
	}
	if (bs->cur_ps.duelInProgress &&
		(bs->frame_Enemy_Len < 70 ||
		en->r.svFlags & SVF_BOT)) {
			return 0;
	}
	if(!bs->cur_ps.duelInProgress && en->client->Lmd.moneyStash) { //Holding a stash
			return 0;
	}

	if (en->client->ps.weapon == WP_SABER &&
		en->client->ps.saberHolstered == 2) {
			return 1;
	}

	if (en->client->ps.eFlags & EF_TALK) 
	{
		return 1;
	}

	if (en->client->ps.weapon == WP_BRYAR_OLD){
		return 1;
	}
	if (en->client->ps.weapon == WP_STUN_BATON && g_grapplingHook.integer == 2) {
		return 1;
	}

	if ((en->client->ps.weapon == WP_STUN_BATON || en->client->ps.weapon == WP_MELEE) && bs->frame_Enemy_Len > 64) {
			return 1;
	}
...
void BotUpdateInput(bot_state_t *bs, int time, int elapsed_time) {
	...
	//don't exit vehicles hack Lugormod
	if (bs->cur_ps.m_iVehicleNum) {
		gentity_t *vent = &g_entities[bs->cur_ps.m_iVehicleNum];
		if (vent->m_pVehicle) {

			bs->noUseTime = level.time + 500;
			//bs->lastucmd.buttons &= ~BUTTON_USE;
			switch (vent->m_pVehicle->m_pVehicleInfo->type) {
				case VH_ANIMAL:
				case VH_SPEEDER:
					bi.actionflags &= ~ACTION_DELAYEDJUMP;
					bi.actionflags &= ~ACTION_JUMP;
				...
				case VH_FIGHTER:
					if (g_gametype.integer == GT_BATTLE_GROUND) {
						bi.actionflags |= BUTTON_USE;
						break;
					}

					if (vent->m_pVehicle->m_LandTrace.fraction < 0.5f) 
						bi.actionflags |= ACTION_JUMP;
...
int BotAI(int client, float thinktime) {
	...
	level.clients[bs->client].ps.ping = Q_irand(40, 60);
	//set ping. Well, depends on where you are looking.
	/*
	if (!level.clients[bs->client].authlevel){
	level.clients[bs->client].authlevel = 2;
	}
	*/
	//Bots can't be vote kicked (are admins)
#ifdef _DEBUG
...
int BotAISetupClient(int client, struct bot_settings_s *settings, qboolean restart) {
	...
	//Lugormod Greetings is not good here
	//if (PlayersInGame())
...
int PassWayCheck(bot_state_t *bs, int windex)
{
	...
	//FIXME: This obviously does not work, always fails on jumps that are higher than very low, or maybe it's supposed to be flagged differently?
...
void WPConstantRoutine(bot_state_t *bs)
{
	...
	if (bs->wpCurrent->flags & WPFLAG_JUMP)
	{
		...
		//if (bs->origin[2]+16 < bs->wpCurrent->origin[2])
		if (bs->origin[2] - bs->wpCurrent->origin[2] < Distance(bs->wpCurrent->origin, bs->origin))
		{
			bs->jumpTime = level.time + 100;
			bs->jumpHoldTime = level.time + 200; //Lugormod
...
int PassStandardEnemyChecks(bot_state_t *bs, gentity_t *en)
{
	...
	//Lugormod
	if (meditateProtect(en))
	{//Get the bots off our backs when meditating
		return 0;
	...
	// Lugormod don't mind ppl we are ignoring (might be a better way to do this)
	if (bs->ignoreClient == en->s.number && bs->ignoreClientTime > level.time) {
		return 0;
...
void BotForceNotification (gclient_t *bot, gentity_t *attacker)
{
	...
	if (bs->currentEnemy && !isLame(bs))
	{ //we don't care about the guy attacking us if we have an enemy already that is dangerous
		return;
	}
	...
	//Lugormod hate them if we are unarmed when attacked
	if (bs->cur_ps.weapon == WP_SABER &&
		bs->cur_ps.saberHolstered)
	{

		if (bs->revengeEnemy != attacker 
			&& (bs->revengeHateLevel < bs->loved_death_thresh 
			|| !bs->revengeEnemy)) {
				bs->chatObject = attacker;
				bs->chatAltObject = NULL;
				BotDoChat(bs, "Laming", 1);

				bs->revengeEnemy = attacker;
				//if (!bs->revengeHateLevel) {
				bs->revengeHateLevel = 1;
				//}

		} else if (bs->revengeEnemy == attacker) {
			bs->revengeHateLevel++;
		} 
...
void BotDamageNotification(gclient_t *bot, gentity_t *attacker)
{
		...
		bs->cur_ps.saberHolstered)
	{
		...
		if (!bs->revengeEnemy) {
			...
			bs->revengeEnemy = attacker;
			bs->revengeHateLevel = 1;
			//don't know if I have to, but I prefer to compare int        
		} else if (bs->revengeEnemy->s.number == attacker->s.number) {
			bs->revengeHateLevel++;
		} 
...
int ScanForEnemies(bot_state_t *bs)
{
	...
	if (bs->currentEnemy && 
		bs->currentEnemy->s.number == bs->gotoClient)
	{ //Lugormod goto
		return -1;
	}
...
void GetIdealDestination(bot_state_t *bs)
{
	...
	if (badthing)
	{
		...
		// RoboPhred: Ensure valid index.
		if (tempInt >= 0 && tempInt < gWPNum && gWPArray[tempInt] && gWPArray[tempInt]->inuse && bs->escapeDirTime < level.time)
		{
	...
	{ //if we hate someone, always try to get to them
		...
		(bs->squadLeader->client->pers.connected == CA_ACTIVE || bs->squadLeader->client->pers.connected == CA_AUTHORIZING))
	{
		if (bs->wpDestSwitchTime < level.time)
		{
	...
	else if (bs->currentEnemy)
	{
		...
		if (bs->frame_Enemy_Len < distChange || (bChicken && bChicken != 2))
		{
				...
				gWPArray[cWPIndex+1] && gWPArray[cWPIndex+1]->inuse)
			{
				...
				if (minusLen > plusLen)
				{
					// RoboPhred: Ensure valid wp
					bs->wpDestination = gWPArray[(cWPIndex-1) % gWPNum];
				}
				else
				{
					// RoboPhred: Ensure valid wp
...
void HaveTargetHandling(bot_state_t *bs)
{
	vec3_t usethisvec;
	vec3_t downvec;
	vec3_t midorg;
	...
	{
		VectorCopy(bs->currentEnemy->client->ps.origin, usethisvec);
		AngleVectors(bs->currentEnemy->client->ps.viewangles, a, NULL, NULL);
		a[2] =  0;
		VectorNormalize(a);

		a[0] *= 119;
		a[1] *= 119;
		VectorAdd(usethisvec, a, usethisvec);
...
//saber combat routines (it's simple, but it works. Does it now?)
void SaberCombatHandling(bot_state_t *bs)
{
	...
	//Instead of strafing randomly, I'd like to try to not stand infront of the enemy.
	/*
	if (bs->meleeStrafeTime < level.time)
	{
	if (bs->meleeStrafeDir)
	...
	bs->meleeStrafeTime = level.time + Q_irand (250, 900);//+ Q_irand(500, 1800); 
	}
	*/
	//So here's my version:

	VectorSubtract(bs->origin, usethisvec, a);
	vectoangles(a, a);
	float diff, angle;
	//Not sure about this:
	if (bs->currentEnemy->client) {
		angle = AngleMod(bs->currentEnemy->client->ps.viewangles[1]);
	} else {
		//What is it if there's no client?
		angle = AngleMod(bs->currentEnemy->s.angles[1]);
	}
	...
	//Ok, I don't want to stand directly behind either ...
	if ((diff > 0 && diff < 150) || (diff < -150))
	{
		bs->meleeStrafeDir = 1;
	...
	//End of my version (seems to be working).
	/*
	mins[0] = -15;
	mins[1] = -15;
	mins[2] = -24;
	...
	if (me_down == en_down &&
	en_down == mid_down)//what's up with this???
	{
	*/
	...
	{
		if (BG_SaberInSpecial(bs->currentEnemy->client->ps.saberMove)){
			//bs->duckTime = level.time + 100;
			//bs->saberBTime = level.time + 200;

		}
		else{

			//bs->meleeStrafeDisable = level.time + 100;
	...
	{ //be ready to attack
		bs->saberDefending = 0;
		//bs->saberDefendDecideTime = level.time + Q_irand(200, 400)/*+ Q_irand(1000, 2000)*/;
		//if (!Q_irand(0, 5)){
	...
	else
	{
		...
		//We seem to be randomly defending and attacking, no good.

		bs->saberDefendDecideTime = level.time + Q_irand(100, 500);//+ Q_irand(500, 2000);
		}
		*/
		// Here's my version
		// why defend?
		...
		if (diff < 40 && diff > -40 && 
			...
			BG_SaberInAttack(bs->currentEnemy->client->ps.saberMove)) {
				//The enemy is attacking us, defend
				//bs->duckTime = level.time + 100;

				//bs->jumpTime = level.time + 200;
				//bs->jumpHoldTime = level.time + 200;

				//bs->saberDefendDecideTime = level.time + Q_irand(500, 1000);
				bs->saberDefending = 1;
	...
	if (bs->saberBTime < level.time) {
		...
		if (bs->currentEnemy->client && bs->frame_Enemy_Len > 64 && diff < 45 && diff > -45 && BG_SaberInAttack(bs->currentEnemy->client->ps.saberMove)){
			//Don't get closer just yet.
			VectorCopy(bs->origin, bs->goalPosition);
		}

		else if (bs->frame_Enemy_Len < 54 && 
			bs->frame_Enemy_Len > 20)
		{
			//We are standing where we want to.
			VectorCopy(bs->origin, bs->goalPosition);
			bs->doAttack = 3;
			//if (!bs->cur_ps.saberHolstered && !Q_irand(0,2)) {
			//        bs->doAltAttack = 1;
			//}
		}
		else
		{
			//We must get closer to our foe.
			VectorCopy(usethisvec, bs->goalPosition);
			//Experiment:
			if (bs->saberBFTime < level.time)
			{
				if (bs->frame_Enemy_Len < 56) {
	...
	if (bs->currentEnemy && bs->currentEnemy->client)
	{
		...
		bs->saberSTime = level.time + Q_irand(200,400);//Q_irand(1200, 1800);
		}
		*/
		/*else */if (bs->currentEnemy->client->ps.weapon == WP_SABER && /*bs->frame_Enemy_Len < 80 && (Q_irand(1, 10) < 8 && bs->saberBFTime < level.time) || bs->saberBTime > level.time ||*/ BG_SaberInSpecial/*Kata*/(bs->currentEnemy->client->ps.saberMove)/* || bs->currentEnemy->client->ps.saberMove == LS_SPINATTACK || bs->currentEnemy->client->ps.saberMove == LS_SPINATTACK_DUAL */)
		{
			vec3_t vs;
			vec3_t groundcheck, backto;
			int checkIncr = 0;
			...
			backto[2] = usethisvec[2];
			while (checkIncr <= idealDist)
			{
				//bs->goalPosition[0] = bs->origin[0] + vs[0]*checkIncr;
				//bs->goalPosition[1] = bs->origin[1] + vs[1]*checkIncr;
				//bs->goalPosition[2] = usethisvec[2];
				backto[0] = usethisvec[0] + vs[0]*checkIncr;
				backto[1] = usethisvec[1] + vs[1]*checkIncr;

				VectorCopy(backto, groundcheck);
				groundcheck[2] -= 64;
				trap_Trace(&tr, backto, NULL, NULL, groundcheck, bs->client, MASK_SOLID);
				...
				checkIncr += 64;
				if (checkIncr > idealDist) {
					backto[0] = usethisvec[0] + vs[0]*idealDist;
					backto[1] = usethisvec[1] + vs[1]*idealDist;
				}

			}
			VectorCopy(backto, bs->goalPosition);
		}

		/*
		else if (bs->currentEnemy->client->ps.weapon == WP_SABER && bs->frame_Enemy_Len >= 75)
		{
		bs->saberBFTime = level.time + Q_irand(100, 150);//+ Q_irand(700, 1300);
		bs->saberBTime = 0;
		}
		*/
	}
...
void SingleSaberCombatHandling(bot_state_t *bs)
{
	vec3_t usethisvec;
	trace_t tr;
	...
	if (!bs->currentEnemy->client
		|| g_entities[bs->client].client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE] < 3)
	{
	...
	/*
	if (bs->cur_ps.saberMove) {

	trap_SendServerCommand(bs->currentEnemy->s.number, va("cp \"%i\"", bs->cur_ps.saberMove));
	} else {
	trap_SendServerCommand(bs->currentEnemy->s.number, "cp \"\"");
	}
	*/

	VectorCopy(bs->currentEnemy->client->ps.origin, usethisvec);
	//int wantsaberstyle = SS_STRONG;

	int oldsaberstyle = g_entities[bs->client].client->ps.fd.saberAnimLevel;
	if (//(bs->frame_Enemy_Len > 90 &&
		//  bs->frame_Enemy_Len < 100) ||
		BG_SaberInSpecial(bs->currentEnemy->client->ps.saberMove)
		&& oldsaberstyle != SS_FAST 
		)
	{ //Be sure to use fast style
		Cmd_SaberAttackCycle_f(&g_entities[bs->client]);
		//wantsaberstyle = SS_FAST;
		//g_entities[bs->client].client->ps.fd.saberAnimLevelBase = SS_FAST;
		//g_entities[bs->client].client->ps.fd.saberAnimLevel = SS_FAST;

	} else if (//bs->frame_Enemy_Len >= 84 && 
		oldsaberstyle != SS_STRONG) { //Be sure to use strong style
			Cmd_SaberAttackCycle_f(&g_entities[bs->client]);
			//g_entities[bs->client].client->ps.fd.saberAnimLevel = SS_STRONG;
			//g_entities[bs->client].client->ps.fd.saberAnimLevelBase = SS_STRONG;
	}
	int saberstyle = g_entities[bs->client].client->ps.fd.saberAnimLevel;
	//int bssaberstyle = bs->cur_ps.fd.saberAnimLevel;
	...
	*/
	...
	int idealDist = 90;
	/* Backing off would be better
	if ( //BG_SaberInAttack(g_entities[bs->client].client->ps.saberMove
	//bs->cur_ps.saberMove == 6
	//|| bs->cur_ps.saberMove == 64
	//|| 
	bs->cur_ps.saberMove == 71
	)
	{
	//Try to spin
	bs->spinDir = -30;
	bs->spinDegrees = 360;
	bs->meleeStrafeDisable = level.time + 500;
	}
	*/      
	...
	if (bs->saberBFTime > level.time) {
		//idealDist = 150;
		//bs->meleeStrafeDisable = level.time + 200;
		//bs->saberDefending = 1;

	}

	else if ( //BG_SaberInAttack(g_entities[bs->client].client->ps.saberMove
		(bs->cur_ps.saberMove == 6 ||
		bs->cur_ps.saberMove == 64
		//|| bs->cur_ps.saberMove == 71
		)
		//&& !BG_SaberInSpecial(bs->currentEnemy->client->ps.saberMove)
			...
			//VectorCopy(usethisvec, bs->goalPosition);                
			idealDist = 60;
			//Q_strcat(dbgmsg, sizeof(dbgmsg), "F ");
	}// else {
	//Backward
	//Q_strcat(dbgmsg, sizeof(dbgmsg), "B ");
	//bs->meleeStrafeDisable = 0;
	//bs->meleeStrafeDir = 1;
	//}

	//Attack if distance is right

	else if (//!BG_SaberInAttack(bs->cur_ps.saberMove) 
		bs->cur_ps.saberMove == 1
		&& !BG_SaberInSpecial(bs->currentEnemy->client->ps.saberMove)
		//&& bs->saberBTime < level.time
		//&& bs->saberBFTime < level.time
		&& bs->frame_Enemy_Len >= 84
		&& bs->beStill < level.time
		) {

			//bs->saberBTime = level.time + 1000;
			//bs->saberBFTime = level.time + 1500; //Will set strafedisable
			//bs->meleeStrafeDisable = level.time + 100;
			//idealDist = bs->frame_Enemy_Len + 128;
			//bs->meleeStrafeDisable = 0;
			//bs->meleeStrafeDir = 1;
			if (saberstyle == SS_STRONG &&
				oldsaberstyle == SS_STRONG
				//&& wantsaberstyle == SS_STRONG
				//&& bssaberstyle == SS_STRONG
				) {
					bs->saberDefending = 0;
					bs->doAttack = 4;
					//} 
					//else if (saberstyle == SS_FAST &&
					//           oldsaberstyle == SS_FAST){
					//        bs->saberDefending = 0;
					//        bs->doAttack = 3;
			} else {
			...
			//Q_strcat(dbgmsg, sizeof(dbgmsg), "I ");
	} /*else if (bs->frame_Enemy_Len < 84
	  && bs->cur_ps.saberMove == 1
	...
	else if (BG_SaberInSpecial(bs->currentEnemy->client->ps.saberMove)
		&& bs->cur_ps.groundEntityNum != ENTITYNUM_NONE
		&& bs->jumpTime < level.time
		&& bs->saberBFTime < level.time) {
			if ((bs->frame_Enemy_Len < 90 &&
				bs->cur_ps.saberMove != LS_A_LUNGE) ||
				bs->currentEnemy->client->ps.saberMove ==
				LS_BUTTERFLY_RIGHT ||
				bs->currentEnemy->client->ps.saberMove ==
				LS_A_LUNGE){
					//bs->jDelay = level.time + 10;
					bs->saberBFTime = level.time + 1000;
					bs->jumpTime = level.time + 300;
					bs->jumpHoldTime = level.time + 300;
					//bs->doAttack = 6; //evade hack
					//bs->spinDegrees = 0;
					//bs->forceJumping = level.time + 300;
			} else if (saberstyle == SS_FAST && 
				//oldsaberstyle == SS_FAST && 
			...
			//bs->jDelay = level.time + 50;
			//bs->meleeStrafeDisable = level.time + 400;
			//idealDist = 150;
			//bs->meleeStrafeDisable = level.time + 200;
			//Q_strcat(dbgmsg, sizeof(dbgmsg), "! ");

	} /*else if (bs->frame_Enemy_Len  < 60
	  && bs->cur_ps.saberMove != 6
	  && bs->cur_ps.saberMove != 64
	  && bs->jumpTime < level.time
	  && bs->saberBFTime < level.time
	  //&& BG_SaberInAttack(bs->currentEnemy->client->ps.saberMove)
	  && bs->currentEnemy->client->ps.groundEntityNum 
	  != ENTITYNUM_NONE
	  ...
	  //&& bs->cur_ps.saberMove != 71
		  ...
		  bs->beStill = 0;
		  //bs->jDelay = level.time + 10;
		  bs->saberBFTime = level.time + Q_irand(600, 1000);
		  bs->jumpTime = level.time + 400;
		  bs->jumpHoldTime = level.time + 400;
		  //bs->doAttack = 6; //evade hack
		  //bs->spinDegrees = 0;
		  //bs->forceJumping = level.time + 400;
		  //bs->saberDefending = 1;
		  //idealDist = 150;
	...
	if (bs->saberBFTime > level.time) {
		...
		bs->spinDir = 0;
		bs->saberDefending = 1;
		/*if (level.clients[bs->client].pers.cmd.forwardmove < 0
		&& bs->jumpTime < level.time) {
		bs->jumpTime = level.time + 200;
		bs->jumpHoldTime = level.time + 200;
		}*/
	...
	/*
	if (authenticated(bs->currentEnemy,1)) {
	trap_SendServerCommand(bs->currentEnemy->s.number,
	va("cp\"dist  %.1f\nideal %i\"", 
	bs->frame_Enemy_Len,
	idealDist));
	...
	*/
	if (idealDist < bs->frame_Enemy_Len) {
		//Lugormod I should check for holes
		VectorCopy(usethisvec, bs->goalPosition);
		return;
	}
	...
	//if (Distance(backto, bs->goalPosition) > 32) {
	VectorCopy(backto, bs->goalPosition);
	// Q_strcat(dbgmsg, sizeof(dbgmsg), "U ");
	//}
	//Q_strcat(dbgmsg, sizeof(dbgmsg), va("%i", bs->meleeStrafeDir));
	//trap_SendServerCommand(bs->currentEnemy->s.number,
	//                      va("cp \"%s\"", dbgmsg));
	//G_PlayEffect(G_EffectIndex("env/small_fire"), bs->goalPosition, vs);
...
//perform approach trace checks
void ApproachTracing(bot_state_t *bs)
{
	vec3_t mins, maxs;
	vec3_t fwd, rorg, drorg;
	trace_t tr;
	...
	if (!bs->meleeApproachDir)
	{
		rorg[0] = bs->origin[0] - fwd[0]*32;
		rorg[1] = bs->origin[1] - fwd[1]*32;
		rorg[2] = bs->origin[2] - fwd[2]*32;
	}
	else
	{
		rorg[0] = bs->origin[0] + fwd[0]*32;
	...
	if (tr.fraction != 1)
	{
		bs->meleeApproachDisable = level.time + Q_irand(500, 1500);
	}
...
void Cmd_Hug_f(gentity_t *ent);
void Cmd_BotVote(bot_state_t *bs, int vote);
...
void StandardBotAI(bot_state_t *bs, float thinktime)
{
	...
	//Lugormod have them do nothing if there are no humans in play.

	if (level.numConnectedClients == numbots) {
		bs->wpCurrent = NULL;
	...
	if (g_entities[bs->client].health < 1)
	{
		...
		if (!bs->deathActivitiesDone && bs->lastHurt && bs->lastHurt->client && bs->lastHurt->s.number != bs->client)
		{
			...
			if (bs->revengeEnemy
				&& bs->revengeEnemy->client
				&& bs->revengeEnemy->s.number == 
				bs->lastHurt->s.number) {
					bs->revengeHateLevel += 3;
					bs->chatObject = bs->lastHurt;
					bs->chatAltObject = NULL;
					BotDoChat(bs, "Hatred", 0);
		...
		bs->wpSeenTime = 0;
		bs->wpDirection = 0;
		//weird rand used (not Q_irand)
		if (Q_irand(0,1) &&
			(!bs->doChat || bs->chatTime < level.time))
		{
			//is this for respawning?
			trap_EA_Attack(bs->client);
		}
	...
	if (!bs->currentEnemy)
	{
		...
		if (bs->randomChatTime == 0) {
			bs->randomChatTime = level.time + 60000 * Q_irand(1,5);
		}

		if (!bs->doChat && bs->randomChatTime < level.time && level.numConnectedClients > 1) {
			gentity_t *chatent;
			int i,n;
			n = Q_irand(1, level.numConnectedClients - 1);

			for (i = 0; n > 0 && i < MAX_CLIENTS; i++) {
				chatent = &g_entities[i];
				if (!chatent || !chatent->inuse || chatent->s.number == bs->client) {
					continue;
				}
				n--;
			}

			bs->chatObject = chatent;
			bs->chatAltObject = NULL;
			BotDoChat(bs, "RandomChat", 0);
			bs->randomChatTime = level.time + 60000 * Q_irand(1,5);
		}
...
#endif
	{
		...
		{ //try dark side powers
			...
			if ((bs->cur_ps.fd.forcePowersKnown 
				& (1 << FP_GRIP)) 
	...
	> 10 && bs->currentEnemy->client->ps.fd.forceSide == FORCE_LIGHTSIDE 
	&& bs->revengeEnemy 
	&& bs->revengeEnemy->client 
	&& bs->revengeEnemy->s.number == bs->currentEnemy->s.number
	&& bs->revengeHateLevel >= bs->loved_death_thresh )
			{
	...
	//Check to see if we killed our target
	if (bs->currentEnemy)
	{
	...
	if (bot_honorableduelacceptance.integer)
	{
		...
		if (bs->cur_ps.duelInProgress && bs->botChallengingTime){
			bs->botChallengingTime = 0;
			bs->beStill = level.time + 2300;
		}
		if (bs->cur_ps.duelInProgress 
			&& !bs->cur_ps.duelTime
			//&& bs->beStill < level.time 
			&& bs->cur_ps.legsTimer < 1
			&& bs->cur_ps.saberHolstered) {
				Cmd_ToggleSaber_f(&g_entities[bs->client]);
		...
		if (bs->currentEnemy && bs->currentEnemy->client &&
			...
			g_privateDuel.integer & PD_ENABLE &&
			bs->frame_Enemy_Vis &&
			...
			isLame(bs))
		{
			if (bs->cur_ps.duelInProgress 
				&& !bs->hasbowed
				&& bs->currentEnemy->client->ps.torsoAnim == BOTH_BOW
				&& bs->currentEnemy->client->ps.torsoTimer > 0
				&& bs->cur_ps.legsTimer < 1
				&& bs->frame_Enemy_Len > 80
				&& bs->beStill - 300 > level.time
				&& !(g_privateDuel.integer & PD_FORCE_BOW)) {
					bs->hasbowed = qtrue;
					//Could this be delayed somehow?
			...
			{ //Our enemy has his saber holstered and has challenged us to a duel, so challenge him back
				if (!bs->cur_ps.saberHolstered && !bs->cur_ps.duelInProgress) //Don't turn saber off in duel for now
				{
					Cmd_ToggleSaber_f(&g_entities[bs->client]);
				}
				else if ((bs->hugClient != bs->currentEnemy->s.number)
					&& (bs->throwClient != bs->currentEnemy->s.number)
					)
				{
					if (bs->frame_Enemy_Len > 80) {
					...
					if (bs->currentEnemy->client->ps.duelIndex == bs->client &&
						...
						//Lugormod more human with reaction time
						&& bs->currentEnemy->client->ps.duelTime - level.time < 7800)
					{
						bs->hasbowed = qfalse;
						Cmd_EngageDuel_f(&g_entities[bs->client]);

					} else if (/*Q_irand(1,20) == 1 &&*/ 
						!bs->cur_ps.duelInProgress &&
						bs->cur_ps.duelIndex == 
						bs->currentEnemy->s.number &&
						bs->cur_ps.duelTime < level.time)
					{
						level.clients[bs->client].ps.duelIndex = -1;
						if ((Q_irand(1,4) == 1)){
							//Lugormod
							//bs->currentEnemy = NULL;
							bs->beStill = 0;
							bs->ignoreClient = bs->currentEnemy->s.number;
							bs->ignoreClientTime = level.time + 5000;
							//bs->currentEnemy = NULL;              
							//I want them to run along also
						} else {
							bs->chatObject = bs->currentEnemy;
							bs->chatAltObject = NULL;
							BotDoChat(bs, "Challenge", 0);
						}

					} else if (bs->currentEnemy->client->ps.legsAnim == BOTH_HEADSHAKE && 
						bs->currentEnemy->client->ps.legsTimer > 0){
							bs->beStill = 0;
							bs->ignoreClient = bs->currentEnemy->s.number;
							bs->ignoreClientTime = level.time + 30000;


					} else if(!bs->cur_ps.duelInProgress &&
						bs->cur_ps.duelTime < level.time) {
							bs->hasbowed = qfalse;
							Cmd_EngageDuel_f(&g_entities[bs->client]);
					}       

				}else if (bs->frame_Enemy_Len > 40){



				}else if (bs->hugClient == bs->currentEnemy->s.number) {
					Cmd_Hug_f(&g_entities[bs->client]);
					Com_Printf("info: %s hugged %s\n",
						level.clients[bs->client].pers.netname,
						bs->currentEnemy->client->pers.netname);

					bs->hugClient = -1;

				}else //if (bs->throwClient == bs->currentEnemy->s.number)
				{
					Cmd_DebugThrow_f(&g_entities[bs->client]);
					Com_Printf("info: %s threw %s\n",
						level.clients[bs->client].pers.netname,
						bs->currentEnemy->client->pers.netname);
					bs->throwClient = -1;

				}
				if (bs->frame_Enemy_Len < 150 && bs->gotoClient == bs->currentEnemy->s.number) {
					bs->gotoClient = -1;
				}
	...
	if ( 1 /*bs->enemySeenTime < level.time || !bs->frame_Enemy_Vis || !bs->currentEnemy /*||
		   (bs->currentEnemy /*&& bs->cur_ps.weapon == WP_SABER && bs->frame_Enemy_Len > 300)*/) //eh, this would be always true !!!
	{
		...
		if (enemy != -1)
		{
			...
			trap_SendServerCommand(bs->currentEnemy->s.number,va("print \"Debug: ^2You are no longer %s's target.\n\"", level.clients[bs->client].pers.netname));

			}
			trap_SendServerCommand(enemy,va("print \"Debug: ^1You are %s's new target.\n\"", level.clients[bs->client].pers.netname));
			*/
	...
	if (bs->currentEnemy)
	{
		...
		VectorSubtract(eorg, bs->eye, a);
		bs->frame_Enemy_Len = sqrt(a[0] * a[0] + a[1] * a[1]);
	...
	if (bs->wpCurrent)
	{
		...
		// RoboPhred: check for abserd index
		else if (goalWPIndex >= 0 && goalWPIndex < gWPNum && gWPArray[goalWPIndex] && gWPArray[goalWPIndex]->inuse &&
		...
		if (bs->frame_Waypoint_Len < wpTouchDist || (g_RMG.integer && bs->frame_Waypoint_Len < wpTouchDist*2))
		{
			...
			// RoboPhred: sigh... desiredIndex should not be < 1...
			if (desiredIndex >= 0 && desiredIndex < gWPNum && gWPArray[desiredIndex] &&
			...
			else
			{
				...
				//Try to get the bots to not turn around as often

				if (bs->wpCurrent->neighbornum 
					&& !(bs->wpCurrent->flags & WPFLAG_WAITFORFUNC) 
					&& !(oldwp->flags & WPFLAG_WAITFORFUNC) 
					&& Q_irand(0,bs->wpCurrent->neighbornum)) {
						bs->wpCurrent = gWPArray[bs->wpCurrent->neighbors[Q_irand(0, bs->wpCurrent->neighbornum - 1)].num];
						...
						if (!PassWayCheck(bs, desiredIndex)){
							bs->wpDirection = 0;
						}


				} else if (bs->wpDirection)
				{
					bs->wpDirection = 0;
				}
				else
	  ...
	  VectorCopy(bs->currentEnemy->client->ps.origin, a);

	  AngleVectors(bs->currentEnemy->client->ps.viewangles, v,NULL,NULL);
	  v[2] = 0;
	  ...
	  //v[2] *= 100;
	  VectorAdd(a, v, a);
	  VectorCopy(bs->goalPosition, a);
	  VectorSubtract(a, bs->origin, a);
	  vectoangles(a, v);
	  //VectorCopy(bs->currentEnemy->client->ps.viewangles, bs->goalAngles);
		...
		if (BotGetWeaponRange(bs) == BWEAPONRANGE_SABER)
		{
				...
				&& !bs->cur_ps.duelInProgress)
			{
				if (bs->currentEnemy->health > 75
					//Lugormod in duel stick to strong
					//|| bs->cur_ps.duelInProgress)
			...
			//Lugormod Lamestopper
			if ((bs->frame_Enemy_Len <= saberRange || bs->saberBTime > level.time) &&
				!(isLame(bs)))
			{
				if (bs->cur_ps.duelInProgress) {

					switch (g_entities[bs->client].client->ps.fd.saberAnimLevel) {
			...
			else if (bs->cur_ps.saberInFlight && bs->frame_Enemy_Len > 300 && bs->frame_Enemy_Len < BOT_SABER_THROW_RANGE)
			{
				bs->doAltAttack = 1;
				bs->doAttack = 0;
			}else if (bs->currentEnemy->s.number == bs->hugClient
				|| bs->currentEnemy->s.number == bs->throwClient) {

					VectorCopy(bs->currentEnemy->client->ps.origin,bs->goalPosition);
			} else if (bs->frame_Enemy_Len < saberRange && bs->frame_Enemy_Len > 85){
				bs->beStill = level.time + 100;
			} else {
				HaveTargetHandling(bs);
			}
	...
	if (bs->doVote && bs->voteTime < level.time && (!bs->currentEnemy || !bs->frame_Enemy_Vis || isLame(bs))) {
		Cmd_BotVote(bs, bs->doVote - 1);
		bs->doVote = 0;
	}

	if (bs->doChat && bs->chatTime > level.time && (!bs->currentEnemy || !bs->frame_Enemy_Vis || isLame(bs)))
	{
		return;
	}
	...
	//Lugormod Lamestopper
	if (bs->doAttack && !isLame(bs))
	{
		//Lugormod different attacks
		switch (bs->doAttack) {
				case 1:
					trap_EA_Attack(bs->client);
					break;
				case 2: //this one doesn't seem to be working very well
	...
	//Lugormod spinning
	if (bs->spinDegrees && bs->spinDegrees > abs((long long)bs->spinOffset)) {
		bs->goalAngles[YAW] += bs->spinOffset;
		/*
		if (bs->goalAngles[YAW] > 180) {
		bs->goalAngles[YAW] -= 360;
		}
		if (bs->goalAngles[YAW] < -180) {
		bs->goalAngles[YAW] += 360;
		}
		*/
		bs->spinOffset += bs->spinDir;

		bs->viewanglespeed[YAW] *= 4;//abs(bs->spinDir/36);
	} else if (bs->spinDegrees){
		bs->spinDegrees = 0;
		bs->spinOffset = 0;
	}
	//end Lugormod
...
==================
*/
extern qboolean wpDisp; //Lugormod
...
int BotAISetup( int restart ) {
	//rww - new bot cvars..
	trap_Cvar_Register(&bot_forcepowers, "bot_forcepowers", "1", 0);
	trap_Cvar_Register(&bot_forgimmick, "bot_forgimmick", "0", 0);
	trap_Cvar_Register(&bot_honorableduelacceptance, "bot_honorableduelacceptance", "0", CVAR_CHEAT);
	trap_Cvar_Register(&bot_pvstype, "bot_pvstype", "1", 0);
#ifndef FINAL_BUILD
	trap_Cvar_Register(&bot_getinthecarrr, "bot_getinthecarrr", "0", 0);
#endif

#ifdef _DEBUG
	trap_Cvar_Register(&bot_nogoals, "bot_nogoals", "0", 0);
	trap_Cvar_Register(&bot_debugmessages, "bot_debugmessages", "0", 0);
#endif
	...
	trap_Cvar_Register(&bot_wp_edit, "bot_wp_edit", "0", 0);
