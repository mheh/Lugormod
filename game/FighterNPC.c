...
{
        //Lugormod can only eject when landed or landing
#ifdef QAGAME
        if ( (!g_disableBail.integer
              || forceEject 
              || pEnt != pVeh->m_pPilot
              || FighterIsLanded(pVeh, pVeh->m_pParentEntity->playerState) 
              || FighterIsLanding(pVeh, pVeh->m_pParentEntity->playerState))
             && g_vehicleInfo[VEHICLE_BASE].Eject( pVeh, pEnt, forceEject ) ) {
                return true;
	}
...
qboolean FighterIsLaunching( Vehicle_t *pVeh, playerState_t *parentPS )
{
	...
	if ( FighterOverValidLandingSurface( pVeh )
#ifdef QAGAME//only do this check on GAME side, because if it's CGAME, it's being predicted, and it's only predicted if the local client is the driver
             && pVeh->m_pVehicleInfo->Inhabited( pVeh )//has to have a driver in order to be capable of launching.
#endif
...
{
        //Lugormod size scaling
        /*
        float scaleh;
        
        if (pVeh->m_pParentEntity->modelScale[2] > 0) {//->client->ps.iModelScale) {
                scaleh = pVeh->m_pParentEntity->modelScale[2];
        } else {
                scaleh = 1.0f;
        }
        */
...
//a predicted event or only doing it game-side. -rich
	...
	*/
	...
	{//pilot jumped out while we were moving forward (not landing or landed) so just keep the throttle locked
		...
		//pVeh->m_ucmd.forwardmove = 127;
                //Lugormod try to get it to land by itself.
                pVeh->m_ucmd.forwardmove = -127;
                if (parentPS->speed > MIN_LANDING_SPEED) {
                        pVeh->m_ucmd.upmove = -127;
                } else {
                ...
                if (pVeh->m_vOrientation[PITCH] > 180) {
                        pVeh->m_vOrientation[PITCH] -= 360;
                } else if (pVeh->m_vOrientation[PITCH] < -180) {
                        pVeh->m_vOrientation[PITCH] += 360;
                }
                
                
                if (pVeh->m_vOrientation[PITCH] > 0) {
                        pVeh->m_vOrientation[PITCH] -= 1;
                } else if (pVeh->m_vOrientation[PITCH] < 0) {
                        pVeh->m_vOrientation[PITCH] += 1;
                }
...
*/
	...
	*/
	...
	{
		float maxDif = pVeh->m_pVehicleInfo->turningSpeed*FIGHTER_TURNING_MULTIPLIER / scaleh;
...
void FighterYawAdjust(Vehicle_t *pVeh, playerState_t *riderPS, playerState_t *parentPS)
{
	...
	{
		float s = parentPS->speed;
		float maxDif = pVeh->m_pVehicleInfo->turningSpeed*0.8f / scaleh; //magic number hackery
...
static void AnimateVehicle( Vehicle_t *pVeh )
{
	...
	qboolean isLanding = qfalse;
        qboolean isLanded = qfalse;
#ifdef _JK2MP
