...
bool ValidateBoard( Vehicle_t *pVeh, bgEntity_t *pEnt )
{
	...
	{//already have a driver!
		...
		{//I know, I know, this should by in the walker's validateboard()
						//Com_Printf("info: %s is trying to steal an at-st\n", ent->client->pers.netname);
					  
			if ( !ent->client 
							 || ent->client->ps.groundEntityNum 
							 != parent->s.number 
							 //|| pVeh->m_iBoarding != 0
								) // not sure about 0
			{//can only steal an occupied AT-ST if you're on top (by the hatch)
								//Com_Printf("info: %s is not standing on the at-st\n", ent->client->pers.netname);
...
bool Board( Vehicle_t *pVeh, bgEntity_t *pEnt )
{
	...
	// ALWAYS let the player be the pilot.
		//Lugormod Try to prevent boarding while duelling
	...
	if ( ent->s.number < MAX_CLIENTS )
	{
		...
		else if ( pVeh->m_iNumPassengers < pVeh->m_pVehicleInfo->maxPassengers )
		{
			...
			pVeh->m_iNumPassengers++;
						//Lugormod tell the pilot
						trap_SendServerCommand(
								pVeh->m_pPilot->s.number, 
								va("cp \"%s Boarded.\n%i passengers in ship.\"",
								   ent->client->pers.netname,
								   pVeh->m_iNumPassengers));
						
		}
				//Lugormod handle stealing
				else if (pVeh->m_pVehicleInfo->type != VH_FIGHTER) {
						//trap_SendServerCommand(
						//       ent-g_entities,
						//       va("print \"DEBUG: %i (Don't mind this)\n\"",
						//          pVeh->m_iBoarding));
 
						//Com_Printf("info: %s is trying kick previous pilot.\n", ent->client->pers.netname);
						
						if (pVeh->m_pVehicleInfo->Eject(
									pVeh, pVeh->m_pPilot, qtrue )) {
								pVeh->m_pVehicleInfo->SetPilot( 
										pVeh,(bgEntity_t *)ent );
						} else {
								return qfalse;
						}
...
//Com_Printf("info: try eject %s\n", ent->client->pers.netname);
	...
	VectorCopy(ent->maxs, vEntMaxs);
#endif
		int i, oldOwners[VEH_MAX_PASSENGERS];
		// Lugormod Passengers should be able to exit.
		for ( i = 0; i < pVeh->m_pVehicleInfo->maxPassengers; i++ )
		{
				if (!pVeh->m_ppPassengers[i] 
					|| !((gentity_t *)pVeh->m_ppPassengers[i])->inuse) {
						continue;
				} 
				oldOwners[i] = 
						((gentity_t *)pVeh->m_ppPassengers[i])->r.ownerNum;
				((gentity_t *)pVeh->m_ppPassengers[i])->r.ownerNum = ent->s.number;
		}
	...
	if ( !forceEject )
	{
		if ( !( pVeh->m_iBoarding == 0 || pVeh->m_iBoarding == -999 || ( pVeh->m_iBoarding < -3 && pVeh->m_iBoarding >= -9 ) ) )
		{
			...
			return false;
				...
				//Lugormod can't jump out of a fighter in the air.
				//FIXME a cvar for disabling this
				/*
				if (pVeh->m_pVehicleInfo->type == VH_FIGHTER
					&& (gentity_t *)pVeh->m_pPilot == ent 
					//&& pVeh->m_LandTrace.fraction > 0.0012
					//&& ent->client->ps.speed > 1
					&& VectorLength(g_entities[ent->client->ps.m_iVehicleNum].s.pos.trDelta) > 5 
					//&& g_entities[ent->client->ps.m_iVehicleNum].client->ps.groundEntityNum == ENTITYNUM_NONE
					//&& !authenticated (ent, 3)
						){
						//Disp(ent, va("vel: %f", VectorLength(g_entities[ent->client->ps.m_iVehicleNum].s.pos.trDelta) > 5));
						Disp(ent, "You must land first.");
						return false;
				}
				*/
...
getItOutOfMe:
	...
	{
		//Com_Printf("info: %s is the pilot.\n", ent->client->pers.netname);
		

#ifdef _JK2MP
		int j = 0;

		//RoboPhred: Check to see if our parent is cloaking us.
		if(parent->client->ps.powerups[PW_CLOAKED] && ((gentity_t *)pVeh->m_pPilot)->client->ps.powerups[PW_CLOAKED]) {
			parent->client->ps.powerups[PW_CLOAKED] = 0;
			G_Sound( parent, CHAN_ITEM, G_SoundIndex("sound/chars/shadowtrooper/cloak.wav") );
		}
#endif
		...
		while (j < pVeh->m_iNumPassengers)
		{
			...
			{
								//Com_Printf("info: passenger %i.\n", j);
				int k = 1;
				pVeh->m_pVehicleInfo->SetPilot( pVeh, pVeh->m_ppPassengers[j] );
				//Com_Printf("info: set pilot done.\n");
				...
				while (k < pVeh->m_iNumPassengers)
				{
					...
					{ //move down
						...
						//Server just needs to tell client which passenger he is
												...
												if (pVeh->m_ppPassengers[k-1] &&
													((gentity_t *)pVeh->m_ppPassengers[k-1])->client )
						{
					...
					k++;
								...
								//Com_Printf("info: rearrange done.\n");
	...
	{
		pVeh->m_pOldPilot = 0;
				//Com_Printf("info: %s is the old pilot???\n", ent->client->pers.netname);
	}
	else
	{
		int i;
		...
		for ( i = 0; i < pVeh->m_pVehicleInfo->maxPassengers; i++ )
		{
						//Com_Printf("info: check if %s is a passenger.\n", ent->client->pers.netname);

			// If we found him...
			if ( (gentity_t *)pVeh->m_ppPassengers[i] == ent )
			{
								//Com_Printf("info: %s is a passenger.\n", ent->client->pers.netname);
				...
				//Lugormod tell the pilot
				if (pVeh->m_pPilot) {
					trap_SendServerCommand(pVeh->m_pPilot->s.number, 
						va("cp \"%s Ejected.\n%i passengers in ship.\"",
							ent->client->pers.netname,
							pVeh->m_iNumPassengers));
				}
		...
		{
						//Com_Printf("info: %s not on the ship.\n", ent->client->pers.netname);
			return false;
		}
	}

		//Com_Printf("info: What now?\n", ent->client->pers.netname);
			...
			if ( pVeh->m_ppPassengers[i] )
			{
				...
				{//Kill them, too
					...
					G_Damage( rider, NULL, NULL, NULL, rider->s.origin, 10000, 0, MOD_SUICIDE );//FIXME: proper MOD?  Get from vehicle?
										//trap_SendServerCommand(rider->s.number, "cp \"Passenger death\"");
...
maintainSelfDuringBoarding:
	...
	pVeh->m_pVehicleInfo->ProcessOrientCommands( pVeh );
	// Need to copy orientation to our entity's viewangles so that it renders at the proper angle and currentAngles is correct.
	//if (pVeh->m_pVehicleInfo->type == VH_WALKER && pVeh->m_pPilot &&
		//    pVeh->m_pPilot->playerState) {
		//        SetClientViewAngle(parent, pVeh->m_pPilot->playerState->viewangles);
		//} else {
		SetClientViewAngle( parent, pVeh->m_vOrientation ); 
		//}
...
static bool UpdateRider( Vehicle_t *pVeh, bgEntity_t *pRider, usercmd_t *pUmcd )
{
	...
	if ( pUmcd->buttons & BUTTON_USE && pVeh->m_pVehicleInfo->type!=VH_SPEEDER)
	{
		...
		{
						//Disp(rider, "Regular Eject");
						// If going too fast, roll off.
						//Lugormod this isn't the speed of the vehicle,
						//so what is this?
		...
		{
						//Com_Printf("info: I don't think this ever happens\n");
